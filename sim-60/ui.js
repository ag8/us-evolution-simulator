...
function runNiceStep() {
    ...
    if (step === 0) {
        text = "Welcome to Andrew's United States Simulator!"
    }
    if (step === 1) {
    ...
    if (step === 3) {
        text = "All right, let's get started. Here is the current United States of America.";
        colorFunction();
    }
    if (step === 4) {
    ...
    if (step === 15) {
        text = "All, right, let's see what happens when the threshold isn't as unreasonably high&mdash;let's try, say, 50 points.";
        threshold = 0.50;
    }
    if (step > 16 && step < 30) {
        runStep();
    ...
    if (step === 31) {
        ...
        text = "For many years now, the <a href=\"https://www.greateridaho.org/\">Greater Idaho project</a> has been trying to move Oregon's border west, and have the eastern counties join Idaho. This simulation shows that these counties have a point---they vote more than 50 points closer to Idaho than to Oregon! In fact, every county that voted to join Idaho in real life did indeed join Idaho here; furthermore, Douglas and Josephine counties, which voted against joining Oregon in real life, did not join Idaho in the simulation either! Thus, 50% seems like a reasonable \"realistic\" threshold for whether counties <i>actually</i> want to leave their state, to the point of voting in favor of it.";
    }
    if (step === 32) {
        animateViewBox("250 190 200 100");
        document.getElementById("c08123").style.fill = '#333';
        text = "However, Weld County, CO, which in real life has voted to join Wyoming, has not yet joined Wyoming in the simulation. So, let's crank the threshold down to <b>40 points</b>, and see what happens.";
    }
    if (step === 33) {
        animateViewBox("0 0 989.98 627.07");
        text = "Here we go!";
